...
TEST(ArchiveHelperTest, InvalidInputPath)
{
    try
    {
        base::utils::ArchiveHelper::decompress("./nonexistent_file");
    }
...
TEST(ArchiveHelperTest, InvalidFormat)
{
    ...
    try
    {
        base::utils::ArchiveHelper::decompress(BASE_EXAMPLE1_PATH);
    }
    catch (const std::runtime_error& e)
    {
...
{
    base::utils::ArchiveHelper::decompress(COMPRESSED_MULTIPLE_FILES_PATH);
...
{
    base::utils::ArchiveHelper::decompress(COMPRESSED_DIR_PATH);
...
TEST(ArchiveHelperTest, SuccessfulDecompressionDirectoryCustomTargetPath)
{
    ...
    base::utils::ArchiveHelper::decompress(COMPRESSED_DIR_PATH, stop, OUTPUT_DIR_PATH.string());
...
{
    const bool stop = false;
    base::utils::ArchiveHelper::decompress(COMPRESSED_DIR_PATH, stop, OUTPUT_NESTED_DIRS_PATH.string());
...
TEST(ArchiveHelperTest, SuccessfulDecompressionExtractOnly)
{
    ...
    const bool stop = false;
    base::utils::ArchiveHelper::decompress(COMPRESSED_DIR_PATH, stop, OUTPUT_DIR_PATH.string(), extractOnly);
